...
// only uses the stable version of the api. For more information about the available versioning see: https://iexcloud.io/docs/api/#api-versioning
...
func (c *IexCloudClient) GetStock(symbol string) (*Stock, error) {

	req, err := http.NewRequest("Get", fmt.Sprintf("%s/stock/%s/quote?token=%s", BaseURL, symbol, c.apiKey), nil)
	if err != nil {
		return nil, err
	}
